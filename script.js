/**
 * Cube Timer Logic
 * Version: 1.2.0 (Optimized Storage, Graph, Rendering)
 */
...
const UPDATE_HISTORY = [
    { date: "2025.12.23", ver: "V1.2.0", content: ["데이터 저장 구조 최적화 (세션별 분할)", "그래프 정보 표시 (최고/최저/평균)", "타이머 렌더링 최적화 (rAF 적용)"] },
    { date: "2025.12.23", ver: "V1.1.2", content: ["Square-1 스크램블 로직 구현"] },
    { date: "2025.12.22", ver: "V1.1.1", content: ["스페이스바를 통한 측정 불가 현상 수정"] },
    { date: "2025.12.21", ver: "V1.1", content: ["모바일 UI 개선", "인스펙션 기능 추가"] },
...
function initEventListeners() {
    ...
    window.addEventListener('keydown', e => {
        if(editingSessionId || document.activeElement.tagName === 'INPUT') {
            if(e.code === 'Enter' && document.activeElement === UI.manualInput) { /* Allow enter */ } 
            else { return; }
        }
    ...
    window.addEventListener('keyup', e => {
        if(e.code === 'Space' && !editingSessionId) handleEnd(e);
    });
    ...
    // Settings Toggles
    UI.precisionToggle.onchange = e => { precision = e.target.checked ? 3 : 2; updateUI(); UI.timer.innerText = (0).toFixed(precision); saveData(); };
    UI.avgModeToggle.onchange = e => { isAo5Mode = e.target.checked; updateUI(); saveData(); };
...
function handleEnd(e) {
    ...
    if (isBtConnected) {
        if (isInspectionMode && inspectionState === 'none') startInspection();
        return;
    }
...
function generateScramble() {
    ...
    if (currentEvent === 'minx') {
        ...
        dials.forEach(d => { res.push(`${d}${Math.floor(Math.random() * 12) - 5 >= 0 ? '+' : ''}${Math.floor(Math.random() * 12) - 5}`); });
        res.push("y2");
        const dials2 = ["U", "R", "D", "L", "ALL"];
        dials2.forEach(d => { res.push(`${d}${Math.floor(Math.random() * 12) - 5 >= 0 ? '+' : ''}${Math.floor(Math.random() * 12) - 5}`); });
        let pins = [];
        ...
        if (currentEvent === '333bf') {
            ...
            for (let i = 0; i < wideMoveCount; i++) res.push(wideMoves[Math.floor(Math.random() * wideMoves.length)] + suffixes[Math.floor(Math.random() * 3)]);
        }
        currentScramble = res.join(" ");
    }
...
window.openUpdateLog = () => { renderUpdateLog(); document.getElementById('updateLogOverlay').classList.add('active'); };
...
window.useThisScramble = () => { 
    ...
    if(s){ currentScramble = s.scramble; UI.scramble.innerText = currentScramble; closeModal(); } 
};
...
function copyToClipboard(text) {
    ...
    try { document.execCommand('copy'); alert("Copied!"); } catch (err) { console.error('Copy failed'); }
...
function saveData() {
    ...
    const meta = {
        sessions: sessions,
        settings: { 
            precision, isAo5Mode, currentEvent, holdDuration, 
            isDarkMode: document.documentElement.classList.contains('dark'),
            isWakeLockEnabled, isInspectionMode 
        }
    };
...
function applySettings(settings) {
    ...
    else { UI.holdDurationSlider.value = holdDuration / 1000; UI.holdDurationValue.innerText = UI.holdDurationSlider.value + "s"; }
...
window.createNewSession = () => { 
    ...
    renderSessionList(); updateUI(); saveData(); resetPenalty(); 
};
window.switchSession = (id) => { 
    sessions[currentEvent].forEach(s => s.isActive = (s.id === id)); 
    renderSessionList(); updateUI(); saveData(); resetPenalty(); closeSessionModal(); 
};
...
function initCube(n = 3) { cubeState = { n }; ['U','D','L','R','F','B'].forEach(f => cubeState[f] = Array(n*n).fill(COLORS[f])); }
...
function applyMove(move) {
    ...
    for(let r=0; r<reps; r++) { for(let l=1; l<=layer; l++) { if(l===1) rotateFaceMatrix(base); /* Face Rotation */
...
window.changeEvent = (e) => {
    ...
    if (['666', '777', '333bf', '444bf', '555bf', '333mbf'].includes(e)) { isAo5Mode = false; UI.avgModeToggle.checked = false; }
    else { isAo5Mode = true; UI.avgModeToggle.checked = true; }

    if (currentEvent === '333mbf') { UI.scramble.classList.add('hidden'); UI.mbfInputArea.classList.remove('hidden'); }
    else { UI.scramble.classList.remove('hidden'); UI.mbfInputArea.classList.add('hidden'); generateScramble(); }
    
    updateUI(); UI.timer.innerText = (0).toFixed(precision); saveData();
};
...
// [OPTIMIZED] Graph Rendering with Labels & Guidelines
function renderHistoryGraph() {
    ...
    if (filtered.length < 2) { 
        // Just show empty polyline if not enough data
    ...
    // Padding to prevent graph touching edges (10%)
    const padding = (maxTime - minTime) * 0.1;
    const graphMax = maxTime + padding;
    const graphMin = Math.max(0, minTime - padding);
    const range = graphMax - graphMin || 1;

    // 1. Draw Guidelines (Average)
    const avgY = 100 - ((avgTime - graphMin) / range) * 100;
    const avgLine = document.createElementNS("http://www.w3.org/2000/svg", "line");
    avgLine.setAttribute("x1", "0"); avgLine.setAttribute("y1", avgY);
    avgLine.setAttribute("x2", "100"); avgLine.setAttribute("y2", avgY);
    avgLine.setAttribute("stroke", "#94a3b8");
    avgLine.setAttribute("stroke-width", "0.5");
    avgLine.setAttribute("stroke-dasharray", "4");
    svg.appendChild(avgLine);

    // 2. Draw Main Polyline
    const points = filtered.map((s, i) => {
        let t = s.penalty === 'DNF' ? graphMax : (s.penalty === '+2' ? s.time + 2000 : s.time);
        // Cap DNF visually to top
        if (t > graphMax) t = graphMax; 
        
        const x = (i / (filtered.length - 1)) * 100;
        const y = 100 - ((t - graphMin) / range) * 100;
        return `${x},${y}`;
    }).join(' ');
    ...
    // 3. Add Labels (Text)
    const createText = (x, y, text, color="#64748b") => {
    ...
    // Max Label
    svg.appendChild(createText(2, 10, `Worst: ${formatTime(maxTime)}`));
    // Min Label
    svg.appendChild(createText(2, 98, `Best: ${formatTime(minTime)}`));
    // Avg Label (Right aligned)
    const avgText = createText(98, avgY - 2, `Avg: ${formatTime(avgTime)}`, "#3b82f6");
...
function confirmClearHistory() {
    if(confirm(`Clear all history for this session?`)) {
        ...
        updateUI(); saveData(); resetPenalty();
